...
func _on_tile_button_pressed() -> void:
	...
	for node in selection.get_selected_nodes():
		if node is Node3D:
			positionAverage += node.position
			selectedCount += 1
	positionAverage /= selectedCount
	startingPosition = positionAverage
